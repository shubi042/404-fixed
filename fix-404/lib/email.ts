...
type SubcontractorBookingDetails = {
	customerName: string
	customerEmail: string
	phone: string
	address: string
...
export async function sendSubcontractorNotificationEmail(
	booking: SubcontractorBookingDetails,
	subcontractor: SubcontractorInfo,
) {
	if (!subcontractor?.email) {
		console.warn("Subcontractor notification skipped: missing email address")
		return { skipped: true }
	}

	const addons =
		Array.isArray(booking.addons) ? booking.addons.join(", ") : booking.addons || "None"

	return sendContractorBookingEmail(subcontractor.email, subcontractor.name, {
		service: booking.serviceName,
		addons,
		estimatedDuration: booking.estimatedDuration || "3 hours",
		date: booking.date,
		time: booking.time,
